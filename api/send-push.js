...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const { alumnoId, payload } = body;

    if (!alumnoId) {
      return res.status(400).json({ error: 'Falta alumnoId en el cuerpo de la petición.' });
    }
    ...
    if (!vapidPublicKey || !vapidPrivateKey) {
      return res.status(500).json({ error: 'Faltan VAPID_PUBLIC_KEY y VAPID_PRIVATE_KEY.' });
    }
    ...
    if (!sbRes.ok) {
      ...
      return res.status(502).json({ error: 'Error consultando suscripciones en Supabase.', detail: errText });
    }
    ...
    if (!subscriptions || subscriptions.length === 0) {
      ...
      return res.status(200).json({ success: true, message: 'Sin suscripciones activas.', sent: 0 });
    }
