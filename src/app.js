...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- Delegación global: Crear Nueva Rutina / Historial (no depende de re-bind) ---
  if (!window._efProfesorClickDelegated) {
    window._efProfesorClickDelegated = true;
    document.addEventListener('click', (e) => {
      const crearBtn = e.target && e.target.closest && e.target.closest('.btn-crear-rutina-click');
      if (crearBtn) {
        e.preventDefault();
        e.stopPropagation();
        try {
          const alumnoId = crearBtn.dataset.alumnoId || (crearBtn.closest('[data-alumno-id]') && crearBtn.closest('[data-alumno-id]').dataset.alumnoId);
          if (!alumnoId) { console.warn('Sin alumnoId en crear rutina'); return; }
          ...
          console.error('Error al abrir crear rutina:', err);
          alert('No se pudo abrir el formulario: ' + (err && err.message ? err.message : err));
        }
        return;
      }
      const histBtn = e.target && e.target.closest && e.target.closest('.btn-historial-click');
      if (histBtn) {
        e.preventDefault();
        e.stopPropagation();
        try {
          const alumnoId = histBtn.dataset.alumnoId || (histBtn.closest('[data-alumno-id]') && histBtn.closest('[data-alumno-id]').dataset.alumnoId);
          if (!alumnoId) return;
      ...
      const personalizada = e.target && e.target.closest && e.target.closest('#btnRutinaPersonalizada');
      if (personalizada) {
        e.preventDefault();
        e.stopPropagation();
        try {
          appState.rutinaEnEdicionId = null;
          appState.plantillaRutinaSeleccionadaId = null;
          if (typeof initFormBuilderForNew === 'function') initFormBuilderForNew();
      ...
      if (usar) {
        ...
        e.stopPropagation();
        try {
          const id = usar.dataset.plantillaId;
          const plantilla = (store.data.rutinas || []).find(r => String(r.id) === String(id));
          if (!plantilla) { alert('No se encontró esa plantilla.'); return; }
          appState.rutinaEnEdicionId = null;
          appState.plantillaRutinaSeleccionadaId = plantilla.id;
          if (typeof initFormBuilderFromPlantilla === 'function') initFormBuilderFromPlantilla(plantilla);
          else if (typeof initFormBuilderForNew === 'function') initFormBuilderForNew();
          appState.modalActivo = 'crear_rutina';
          if (typeof renderApp === 'function') renderApp();
          ...
          console.error(err);
          alert('Error al usar plantilla: ' + (err.message || err));
        }
        return;
      }
    }, true); // capture phase: gana al handler de la card
  }
      ...
      <header class="app-header">
        <div class="header-main-row">
          ...
          <div class="header-actions">
            ${user ? `
              <span class="badge header-user-badge header-user-desktop ${isProfesor ? 'badge-warning' : 'badge-active'}" title="${user.nombre}">
        ...
        ${user ? `
          <div class="header-user-mobile-bar">
            <span class="badge header-user-badge header-user-mobile ${isProfesor ? 'badge-warning' : 'badge-active'}" title="${user.nombre}">
              ${isProfesor ? '⚡' : '👤'} ${user.nombre}
            </span>
          </div>
        ` : ''}
  ...
  function renderModalElegirOrigenRutina() {
    ...
    let plantillas = [];
    try { plantillas = (typeof store.getPlantillasRutina === 'function') ? (store.getPlantillasRutina() || []) : []; } catch (_e) { plantillas = []; }
  ...
  const CATALOGO_VIDEOS_EJERCICIOS = [
    ...
    { nombre: "Pull over en polea con soga", videoUrl: "https://www.youtube.com/shorts/oOmDvbqJj-Q", gifUrl: "https://static.exercisedb.dev/media/Q2Eu1Ax.gif", alias: ["Pull over en polea con soga", "pull over en polea con soga", "pullover en polea con soga", "pull over polea soga", "pullover polea soga", "pull-over en polea con soga", "pull over en polea con cuerda"] },
    { nombre: "Press de hombro en maquina", videoUrl: "https://www.youtube.com/shorts/BqdJeb6jHko", gifUrl: "https://static.exercisedb.dev/media/dNFYIU1.gif", alias: ["Press de hombro en maquina", "Press de hombro en máquina", "press de hombro en maquina", "press de hombro en máquina", "press hombro maquina", "press hombro máquina", "press de hombros en maquina", "shoulder press maquina"] },
    { nombre: "Patada de gluteo en polea", videoUrl: "https://www.youtube.com/shorts/FclsfZvytqM", gifUrl: "https://static.exercisedb.dev/media/Kpajagk.gif", alias: ["Patada de gluteo en polea", "patada de gluteo en polea", "patada de glúteo en polea", "patada gluteo polea", "patada glúteo polea", "Patada de glúteo", "patada de glúteo", "Patada de gluteo", "patada de gluteo", "glute kickback polea", "patadas de gluteo en polea"] },
    ...
    { nombre: "Patada de gluteo en polea con banco", videoUrl: "https://www.youtube.com/shorts/BNOfVprH6DI", gifUrl: "https://static.exercisedb.dev/media/HEJ6DIX.gif", alias: ["Patada de gluteo en polea con banco", "patada de gluteo en polea con banco", "patada de glúteo en polea con banco", "patada gluteo polea banco", "patada gluteo en polea banco", "glute kickback polea banco"] },
    { nombre: "Abduccion sentado en maquina", videoUrl: "https://www.youtube.com/shorts/xlp6FYwBFLU", gifUrl: "https://static.exercisedb.dev/media/CHpahtl.gif", alias: ["Abduccion sentado en maquina", "Abduccion sentado en máquina", "abduccion sentado en maquina", "abduccion sentado en máquina"] },
    { nombre: "Abs bolitas", videoUrl: "https://www.youtube.com/shorts/Wlw3wvJdhBc", gifUrl: "https://static.exercisedb.dev/media/SLKj2pX.gif", alias: ["Abs bolitas", "abs bolitas", "Abdominales Bolita", "abdominales bolita", "Abdominales bolita", "abdominales bolitas", "abs bolita"] },
    { nombre: "Abs rectos con peso", videoUrl: "https://www.youtube.com/shorts/BFTJUFmiwHM", gifUrl: "https://static.exercisedb.dev/media/s8nrDXF.gif", alias: ["Abs rectos con peso", "abs rectos con peso"] },
    { nombre: "Abs rectos con piernas a 45°", videoUrl: "https://www.youtube.com/shorts/4rttpgLuJR8", gifUrl: "https://static.exercisedb.dev/media/2gPfomN.gif", alias: ["Abs rectos con piernas a 45°", "abs rectos con piernas a 45°"] },
    { nombre: "Aduccion sentado en maquina", videoUrl: "https://www.youtube.com/shorts/vlViIgtkvh4", gifUrl: "https://static.exercisedb.dev/media/oHsrypV.gif", alias: ["Aduccion sentado en maquina", "Aduccion sentado en máquina", "aduccion sentado en maquina", "aduccion sentado en máquina"] },
    { nombre: "Apertura con empuje en banco plano", videoUrl: "https://www.youtube.com/shorts/Fc1UgcryhrA", gifUrl: "https://static.exercisedb.dev/media/yz9nUhF.gif", alias: ["Apertura con empuje en banco plano", "apertura con empuje en banco plano"] },
    { nombre: "Apertura en banco inclinado", videoUrl: "https://www.youtube.com/shorts/CQx4FhzelMU", gifUrl: "https://static.exercisedb.dev/media/ESOd5Pl.gif", alias: ["Apertura en banco inclinado", "apertura en banco inclinado"] },
    { nombre: "Apertura en banco plano", videoUrl: "https://www.youtube.com/shorts/26TsSvfg28U", gifUrl: "https://static.exercisedb.dev/media/yz9nUhF.gif", alias: ["Apertura en banco plano", "apertura en banco plano"] },
    ...
    { nombre: "Arranque a 1 brazo", videoUrl: "https://www.youtube.com/shorts/IVIrE_x5FtE", gifUrl: "https://static.exercisedb.dev/media/6pTkI99.gif", alias: ["Arranque a 1 brazo", "arranque a 1 brazo"] },
    { nombre: "Arranque con barra", videoUrl: "https://www.youtube.com/shorts/-7mUZ2RFAw4", alias: ["Arranque con barra", "arranque con barra"] },
    { nombre: "Biceps con barra", videoUrl: "https://www.youtube.com/shorts/B3Feq30xwpo", gifUrl: "https://static.exercisedb.dev/media/25GPyDY.gif", alias: ["Biceps con barra", "biceps con barra", "bíceps con barra"] },
    { nombre: "Biceps con barra en polea", videoUrl: "https://www.youtube.com/shorts/oJGSW0z5Hew", gifUrl: "https://static.exercisedb.dev/media/G08RZcQ.gif", alias: ["Biceps con barra en polea", "biceps con barra en polea", "bíceps con barra en polea"] },
    { nombre: "Biceps con mancuerna banco inclinado", videoUrl: "https://www.youtube.com/shorts/4LQ0p0ni39Y", gifUrl: "https://static.exercisedb.dev/media/F3xgbjF.gif", alias: ["Biceps con mancuerna banco inclinado", "Biceps con mancuernas banco inclinado", "biceps con mancuerna banco inclinado", "biceps con mancuernas banco inclinado", "bíceps con mancuerna banco inclinado"] },
    { nombre: "Biceps con mancuernas", videoUrl: "https://www.youtube.com/shorts/WrpQYs_n_Pw", gifUrl: "https://static.exercisedb.dev/media/NbVPDMW.gif", alias: ["Biceps con mancuerna", "Biceps con mancuernas", "biceps con mancuerna", "biceps con mancuernas", "bíceps con mancuernas"] },
    { nombre: "Biceps con soga en polea", videoUrl: "https://www.youtube.com/shorts/dj99CeV_UUY", gifUrl: "https://static.exercisedb.dev/media/HPlPoQA.gif", alias: ["Biceps con soga en polea", "biceps con soga en polea", "bíceps con soga en polea"] },
    { nombre: "Biceps en banco scott", videoUrl: "https://www.youtube.com/shorts/Zj0P6gFb9MY", gifUrl: "https://static.exercisedb.dev/media/qOgPVf6.gif", alias: ["Biceps en banco scott", "biceps en banco scott", "bíceps en banco scott"] },
    { nombre: "Biceps en banco scott con mancuernas", videoUrl: "https://www.youtube.com/shorts/L4AQh3lTICk", gifUrl: "https://static.exercisedb.dev/media/jivWf8n.gif", alias: ["Biceps en banco scott con mancuerna", "Biceps en banco scott con mancuernas", "biceps en banco scott con mancuerna", "biceps en banco scott con mancuernas", "bíceps en banco scott con mancuernas"] },
    { nombre: "Biceps martillo", videoUrl: "https://www.youtube.com/shorts/QdEY-VZcPMU", gifUrl: "https://static.exercisedb.dev/media/slDvUAU.gif", alias: ["Biceps martillo", "biceps martillo", "bíceps martillo"] },
    { nombre: "Bicho muerto", videoUrl: "https://www.youtube.com/shorts/_o23Ghklahc", gifUrl: "https://static.exercisedb.dev/media/iny3m5y.gif", alias: ["Bicho muerto", "bicho muerto"] },
    { nombre: "Bisagras oblicuas", videoUrl: "https://www.youtube.com/shorts/EExl_U2GGEk", gifUrl: "https://static.exercisedb.dev/media/XVDdcoj.gif", alias: ["Bisagras oblicuas", "bisagras oblicuas"] },
    ...
    { nombre: "Camilla de isquios 1 pp", videoUrl: "https://www.youtube.com/shorts/Ix7wiKrC7bQ", gifUrl: "https://static.exercisedb.dev/media/UXpKJoq.gif", alias: ["Camilla de isquios 1 pierna", "Camilla de isquios 1 pp", "Camilla de isquios a un pie", "Camilla de isquios unilateral", "Camilla isquios 1 pp", "camilla de isquios 1 pierna", "camilla de isquios 1 pp", "camilla de isquios a un pie", "camilla de isquios unilateral", "camilla isquios 1 pp"] },
    { nombre: "Cargada", videoUrl: "https://www.youtube.com/shorts/7KMYH9gyeEM", gifUrl: "https://static.exercisedb.dev/media/SiWCcTN.gif", alias: ["Cargada", "cargada"] },
    { nombre: "Cargada + segundo tiempo", videoUrl: "https://www.youtube.com/shorts/MrG8DIttWVA", gifUrl: "https://static.exercisedb.dev/media/SGY8Zui.gif", alias: ["Cargada + segundo tiempo", "cargada + segundo tiempo"] },
    { nombre: "Cuadricera", videoUrl: "https://www.youtube.com/shorts/iNvPzf15KKA", gifUrl: "https://static.exercisedb.dev/media/my33uHU.gif", alias: ["Cuadricera", "cuadricera"] },
    { nombre: "Cuadricera 1 pp", videoUrl: "https://www.youtube.com/shorts/OhHvfoomEfY", gifUrl: "https://static.exercisedb.dev/media/my33uHU.gif", alias: ["Cuadricera 1 pierna", "Cuadricera 1 pp", "Cuadricera a un pie", "Cuadricera unilateral", "cuadricera 1 pierna", "cuadricera 1 pp", "cuadricera a un pie", "cuadricera unilateral"] },
    ...
    { nombre: "Encogimiento con barra", videoUrl: "https://www.youtube.com/shorts/xgLOVCSFTAc", gifUrl: "https://static.exercisedb.dev/media/dG7tG5y.gif", alias: ["Encogimiento con barra", "encogimiento con barra"] },
    { nombre: "Encogimiento con mancuernas", videoUrl: "https://www.youtube.com/shorts/0-KrI1Hqpk4", gifUrl: "https://static.exercisedb.dev/media/NJzBsGJ.gif", alias: ["Encogimiento con mancuerna", "Encogimiento con mancuernas", "encogimiento con mancuerna", "encogimiento con mancuernas"] },
    { nombre: "Estocadas fijas en smith", videoUrl: "https://www.youtube.com/shorts/sEE7Qrlf4j4", gifUrl: "https://static.exercisedb.dev/media/HsjbB1z.gif", alias: ["Estocadas fijas en en smith", "Estocadas fijas en smith", "Estocadas fijas smith", "estocada fijas en smith", "estocadas fijas en en smith", "estocadas fijas en smith", "estocadas fijas smith", "zancadas fijas en smith"] },
    { nombre: "Estocadas hacia adelante", videoUrl: "https://www.youtube.com/shorts/IIJA6XbIdyI", gifUrl: "https://static.exercisedb.dev/media/kMzUs9Y.gif", alias: ["Estocadas hacia adelante", "estocada hacia adelante", "estocadas hacia adelante", "zancadas hacia adelante"] },
    ...
    { nombre: "Face pull", videoUrl: "https://www.youtube.com/shorts/5Yu8DTe4BAQ", gifUrl: "https://static.exercisedb.dev/media/tc5dYrf.gif", alias: ["Face pull", "face pull"] },
    { nombre: "Fondos en banco", videoUrl: "https://www.youtube.com/shorts/wN_9d37DO4M", gifUrl: "https://static.exercisedb.dev/media/RrLske5.gif", alias: ["Fondos en banco", "fondos en banco"] },
    { nombre: "Fondos en paralelas", videoUrl: "https://www.youtube.com/shorts/s5BLhbKWD3E", gifUrl: "https://static.exercisedb.dev/media/X6C6i5Y.gif", alias: ["Fondos en paralelas", "fondos en paralelas"] },
    { nombre: "Hip thrust", videoUrl: "https://www.youtube.com/shorts/zaSzxo6xleY", gifUrl: "https://static.exercisedb.dev/media/qKBpF7I.gif", alias: ["Hip thrust", "empuje de cadera", "hip thrust", "hipthrust"] },
    { nombre: "Inferiores carrito", videoUrl: "https://www.youtube.com/shorts/62-5TombKfU", gifUrl: "https://static.exercisedb.dev/media/I3tsCnC.gif", alias: ["Inferiores carrito", "inferiores carrito"] },
    { nombre: "Inferiores con peso", videoUrl: "https://www.youtube.com/shorts/zMJ6Hqv8-sM", gifUrl: "https://static.exercisedb.dev/media/I3tsCnC.gif", alias: ["Inferiores con peso", "inferiores con peso"] },
    { nombre: "Inferiores en paralelas", videoUrl: "https://www.youtube.com/watch?v=fX6JwyyW16o", gifUrl: "https://static.exercisedb.dev/media/ZNgOYQU.gif", alias: ["Inferiores en paralelas", "inferiores en paralelas"] },
    { nombre: "Isquios sentado en maquina", videoUrl: "https://www.youtube.com/shorts/6IvQvWZmEsw", gifUrl: "https://static.exercisedb.dev/media/Zg3XY7P.gif", alias: ["Isquios sentado en maquina", "Isquios sentado en máquina", "isquios sentado en maquina", "isquios sentado en máquina"] },
    { nombre: "Isquios sentado en maquina 1 pp", videoUrl: "https://www.youtube.com/shorts/UNc9k9WBXj8", gifUrl: "https://static.exercisedb.dev/media/Zg3XY7P.gif", alias: ["Isquios sentado en maquina 1 pierna", "Isquios sentado en maquina 1 pp", "Isquios sentado en maquina a un pie", "Isquios sentado en maquina unilateral", "Isquios sentado en máquina 1 pp", "isquios sentado en maquina 1 pierna", "isquios sentado en maquina 1 pp", "isquios sentado en maquina a un pie", "isquios sentado en maquina unilateral", "isquios sentado en máquina 1 pp"] },
    { nombre: "Jalon dorsal 1 brazo", videoUrl: "https://www.youtube.com/shorts/rudpTb4A0X8", gifUrl: "https://static.exercisedb.dev/media/pmnrOp0.gif", alias: ["Jalon dorsal 1 brazo", "jalon dorsal 1 brazo", "jalón dorsal 1 brazo"] },
    { nombre: "Jalon dorsal neutro", videoUrl: "https://www.youtube.com/shorts/Yie3dzpYNtQ", gifUrl: "https://static.exercisedb.dev/media/4c9BhzB.gif", alias: ["Jalon dorsal neutro", "jalon dorsal neutro", "jalón dorsal neutro"] },
    { nombre: "Jalon dorsal prono amplio", videoUrl: "https://www.youtube.com/shorts/sAYU5EvtXSo", gifUrl: "https://static.exercisedb.dev/media/qdRxqCj.gif", alias: ["Jalon dorsal prono amplio", "jalon al pecho", "jalon dorsal prono amplio", "jalon dorsalera", "jalón al pecho", "jalón dorsal prono amplio"] },
    { nombre: "Jalon dorsal supino", videoUrl: "https://www.youtube.com/shorts/Q4e_Ya9BnlM", gifUrl: "https://static.exercisedb.dev/media/xBYcQHj.gif", alias: ["Jalon dorsal supino", "jalon dorsal supino", "jalón dorsal supino"] },
    { nombre: "Peso muerto", videoUrl: "https://www.youtube.com/shorts/WdADxet2RQc", gifUrl: "https://static.exercisedb.dev/media/ila4NZS.gif", alias: ["Peso muerto", "deadlift", "peso muerto"] },
    { nombre: "Peso muerto 1 pp", videoUrl: "https://www.youtube.com/shorts/Z3TRkamr5jA", gifUrl: "https://static.exercisedb.dev/media/gEyURal.gif", alias: ["Peso muerto 1 pierna", "Peso muerto 1 pp", "Peso muerto a un pie", "Peso muerto unilateral", "deadlift 1 pp", "peso muerto 1 pierna", "peso muerto 1 pp", "peso muerto a un pie", "peso muerto unilateral"] },
    { nombre: "Plancha frontal", videoUrl: "https://www.youtube.com/shorts/XCOq6lfsFAk", gifUrl: "https://static.exercisedb.dev/media/h1ezqSu.gif", alias: ["Plancha frontal", "plancha frontal"] },
    ...
    { nombre: "Plancha frontal toco hombro", videoUrl: "https://www.youtube.com/shorts/auypDs3TVeM", gifUrl: "https://static.exercisedb.dev/media/h1ezqSu.gif", alias: ["Plancha frontal toco hombro", "plancha frontal toco hombro"] },
    { nombre: "Plancha frontal con o sin peso", videoUrl: "https://www.youtube.com/shorts/XCOq6lfsFAk", gifUrl: "https://static.exercisedb.dev/media/VBAWRPG.gif", alias: ["Plancha frontal con o sin peso", "plancha frontal con o sin peso", "plancha frontal con peso", "plancha frontal sin peso", "plancha con o sin peso"] },
    { nombre: "Plancha lateral", videoUrl: "https://www.youtube.com/shorts/fNsxKTKfMNI", gifUrl: "https://static.exercisedb.dev/media/5VXmnV5.gif", alias: ["Plancha lateral", "plancha lateral"] },
    { nombre: "Prensa en 45°", videoUrl: "https://www.youtube.com/shorts/NYa0tZCW4fk", gifUrl: "https://static.exercisedb.dev/media/10Z2DXU.gif", alias: ["Prensa en 45°", "prensa en 45°"] },
    { nombre: "Prensa horizontal", videoUrl: "https://www.youtube.com/shorts/W7bL6i1sJo4", gifUrl: "https://static.exercisedb.dev/media/IeDEXTe.gif", alias: ["Prensa horizontal", "prensa horizontal"] },
    ...
    { nombre: "Press de banca a 1 brazo con mancuerna", videoUrl: "https://www.youtube.com/watch?v=N_BzPe7kmdk", gifUrl: "https://static.exercisedb.dev/media/SpYC0Kp.gif", alias: ["Press banca a 1 brazo con mancuerna", "Press de banca a 1 brazo con mancuerna", "Press de banca a 1 brazo con mancuernas", "press banca a 1 brazo con mancuerna", "press de banca a 1 brazo con mancuerna", "press de banca a 1 brazo con mancuernas"] },
    { nombre: "Press de banca declinado", videoUrl: "https://www.youtube.com/shorts/NEOBG2KgVyA", gifUrl: "https://static.exercisedb.dev/media/GrO65fd.gif", alias: ["Press banca declinado", "Press de banca declinado", "press banca declinado", "press de banca declinado"] },
    { nombre: "Press de banca inclinado con barra", videoUrl: "https://www.youtube.com/shorts/g99l4KwY-vo", gifUrl: "https://static.exercisedb.dev/media/3TZduzM.gif", alias: ["Press banca inclinado con barra", "Press de banca inclinado con barra", "press banca inclinado con barra", "press de banca inclinado con barra"] },
    { nombre: "Press de banca inclinado con mancuernas", videoUrl: "https://www.youtube.com/shorts/tcw2c5dtqD4", gifUrl: "https://static.exercisedb.dev/media/ns0SIbU.gif", alias: ["Press banca inclinado con mancuernas", "Press de banca inclinado con mancuernas", "press banca inclinado con mancuernas", "press de banca inclinado con mancuernas"] },
    { nombre: "Press de banca inclinado en smith", videoUrl: "https://www.youtube.com/shorts/XwjuCBcFXQQ", gifUrl: "https://static.exercisedb.dev/media/5v7KYld.gif", alias: ["Press banca inclinado en smith", "Press de banca inclinado en en smith", "Press de banca inclinado en smith", "Press de banca inclinado smith", "press banca inclinado en smith", "press de banca inclinado en en smith", "press de banca inclinado en smith", "press de banca inclinado smith"] },
    { nombre: "Press de banca plano con barra", videoUrl: "https://www.youtube.com/shorts/HzkHpIIo4IA", gifUrl: "https://static.exercisedb.dev/media/EIeI8Vf.gif", alias: ["Press banca plano con barra", "Press de banca plano con barra", "press banca plano con barra", "press de banca plano con barra", "press plano con barra"] },
    { nombre: "Press de banca plano con mancuernas", videoUrl: "https://www.youtube.com/shorts/qW519gsE2M8", gifUrl: "https://static.exercisedb.dev/media/SpYC0Kp.gif", alias: ["Press banca plano con mancuernas", "Press de banca plano con mancuerna", "Press de banca plano con mancuernas", "press banca plano con mancuernas", "press de banca plano con mancuerna", "press de banca plano con mancuernas", "press plano con mancuernas"] },
    { nombre: "Press de banca plano en smith", videoUrl: "https://www.youtube.com/shorts/9_tbUqJ45QU", gifUrl: "https://static.exercisedb.dev/media/trqKQv2.gif", alias: ["Press banca plano en smith", "Press de banca plano en en smith", "Press de banca plano en smith", "Press de banca plano smith", "press banca plano en smith", "press de banca plano en en smith", "press de banca plano en smith", "press de banca plano smith", "press plano en smith"] },
    { nombre: "Press de hombro 1 brazo", videoUrl: "https://www.youtube.com/shorts/i5cvgQveFbY", gifUrl: "https://static.exercisedb.dev/media/84RyJf8.gif", alias: ["Press de hombro 1 brazo", "Press hombro 1 brazo", "press de hombro 1 brazo", "press hombro 1 brazo"] },
    { nombre: "Press de hombro con barra", videoUrl: "https://www.youtube.com/shorts/z_-74FMv5Jg", gifUrl: "https://static.exercisedb.dev/media/kTbSH9h.gif", alias: ["Press de hombro con barra", "Press hombro con barra", "press de hombro con barra", "press hombro con barra"] },
    { nombre: "Press de hombro con barra sentado", videoUrl: "https://www.youtube.com/shorts/AmcpSwUNPYc", gifUrl: "https://static.exercisedb.dev/media/kTbSH9h.gif", alias: ["Press de hombro con barra sentado", "Press hombro con barra sentado", "press de hombro con barra sentado", "press hombro con barra sentado"] },
    { nombre: "Press de hombro con mancuernas", videoUrl: "https://www.youtube.com/shorts/96C2nPejfY4", gifUrl: "https://static.exercisedb.dev/media/A6wtbuL.gif", alias: ["Press de hombro con mancuerna", "Press de hombro con mancuernas", "Press hombro con mancuernas", "press de hombro con mancuerna", "press de hombro con mancuernas", "press hombro con mancuernas"] },
    { nombre: "Press de hombro con mancuernas sentado", videoUrl: "https://www.youtube.com/shorts/R0f2Of6Sl2A", gifUrl: "https://static.exercisedb.dev/media/znQUdHY.gif", alias: ["Press de hombro con mancuerna sentado", "Press de hombro con mancuernas sentado", "Press hombro con mancuernas sentado", "press de hombro con mancuerna sentado", "press de hombro con mancuernas sentado", "press hombro con mancuernas sentado"] },
    { nombre: "Press de pecho en maquina sentado", videoUrl: "https://www.youtube.com/shorts/88kSaeQG21Y", gifUrl: "https://static.exercisedb.dev/media/T0yTjgW.gif", alias: ["Press de pecho en maquina sentado", "Press de pecho en máquina sentado", "Press pecho en maquina sentado", "press de pecho en maquina sentado", "press de pecho en máquina sentado", "press pecho en maquina sentado"] },
    { nombre: "Press de pecho inclinado en maquina", videoUrl: "https://www.youtube.com/shorts/YS6DsKrY8T0", gifUrl: "https://static.exercisedb.dev/media/o17Jfkt.gif", alias: ["Press de pecho inclinado en maquina", "Press de pecho inclinado en máquina", "Press pecho inclinado en maquina", "press de pecho inclinado en maquina", "press de pecho inclinado en máquina", "press pecho inclinado en maquina"] },
    { nombre: "Press frances con barra", videoUrl: "https://www.youtube.com/watch?v=gY-CqZD0Ktc", gifUrl: "https://static.exercisedb.dev/media/iZop9xO.gif", alias: ["Press frances con barra", "press frances con barra"] },
    { nombre: "Press frances con barra W", videoUrl: "https://www.youtube.com/shorts/CAUWI4sNPKk", gifUrl: "https://static.exercisedb.dev/media/h8LFzo9.gif", alias: ["Press frances con barra W", "press frances con barra w"] },
    { nombre: "Press frances con mancuerna", videoUrl: "https://www.youtube.com/shorts/FgN0vyx8jNE", gifUrl: "https://static.exercisedb.dev/media/mpKZGWz.gif", alias: ["Press frances con mancuerna", "Press frances con mancuernas", "press frances con mancuerna", "press frances con mancuernas"] },
    { nombre: "Press frances en polea", videoUrl: "https://www.youtube.com/shorts/wNVVttuAGnM", gifUrl: "https://static.exercisedb.dev/media/uxJcFUU.gif", alias: ["Press frances en polea", "press frances en polea"] },
    { nombre: "Remo con barra", videoUrl: "https://www.youtube.com/shorts/b8FgtZlyEd4", gifUrl: "https://static.exercisedb.dev/media/eZyBC3j.gif", alias: ["Remo con barra", "remo con barra"] },
    { nombre: "Remo en barra T", videoUrl: "https://www.youtube.com/shorts/uLr8HcW_7ig", gifUrl: "https://static.exercisedb.dev/media/aaXr7ld.gif", alias: ["Remo en barra T", "remo en barra t"] },
    ...
    { nombre: "Remo en polea baja 1 brazo", videoUrl: "https://www.youtube.com/shorts/Ol37ocDBdAU", gifUrl: "https://static.exercisedb.dev/media/EIsE3u8.gif", alias: ["Remo en polea baja 1 brazo", "remo en polea baja 1 brazo"] },
    { nombre: "Remo en polea baja neutro", videoUrl: "https://www.youtube.com/shorts/soxtqUNRt6E", gifUrl: "https://static.exercisedb.dev/media/fUBheHs.gif", alias: ["Remo en polea baja neutro", "remo en polea baja neutro"] },
    { nombre: "Remo en polea baja prono", videoUrl: "https://www.youtube.com/watch?v=Vm6E-2tq0bU", gifUrl: "https://static.exercisedb.dev/media/hvV79Si.gif", alias: ["Remo en polea baja prono", "remo en polea baja prono"] },
    ...
    { nombre: "Remo en smith", videoUrl: "https://www.youtube.com/shorts/lTQJFDyQq0s", gifUrl: "https://static.exercisedb.dev/media/JGKowMS.gif", alias: ["Remo en en smith", "Remo en smith", "Remo smith", "remo en en smith", "remo en smith", "remo smith", "Remo con barra en smith", "remo con barra en smith"] },
    { nombre: "Ruedita abdominal", videoUrl: "https://www.youtube.com/shorts/knjliGWvGa4", gifUrl: "https://static.exercisedb.dev/media/NAgVB3t.gif", alias: ["Ruedita abdominal", "ruedita abdominal"] },
    { nombre: "Segundo tiempo", videoUrl: "https://www.youtube.com/shorts/fWQbTvolNqA", gifUrl: "https://static.exercisedb.dev/media/SiWCcTN.gif", alias: ["Segundo tiempo", "segundo tiempo"] },
    { nombre: "Sentadilla barra hexagonal", videoUrl: "https://www.youtube.com/shorts/JfhNkNz59lI", gifUrl: "https://static.exercisedb.dev/media/jQGwmxN.gif", alias: ["Sentadilla barra exagonal", "Sentadilla barra hexagonal", "Sentadilla hex bar", "senatdilla barra hexagonal", "sentadilla barra exagonal", "sentadilla barra hexagonal", "sentadilla hex bar", "sentadillas barra hexagonal"] },
    { nombre: "Sentadilla bulgaras", videoUrl: "https://www.youtube.com/shorts/cytfxsIK_Hk", gifUrl: "https://static.exercisedb.dev/media/gGNQmVt.gif", alias: ["Sentadilla bulgaras", "senatdilla bulgaras", "sentadilla bulgaras", "sentadillas bulgaras"] },
    { nombre: "Sentadilla con barra", videoUrl: "https://www.youtube.com/shorts/7xeLHxobaWs", gifUrl: "https://static.exercisedb.dev/media/qXTaZnJ.gif", alias: ["Sentadilla con barra", "senatdilla con barra", "sentadilla con barra", "sentadillas con barra"] },
    { nombre: "Sentadilla con polea", videoUrl: "https://www.youtube.com/shorts/0vz9HwV4UKg", gifUrl: "https://static.exercisedb.dev/media/f7fnAIB.gif", alias: ["Sentadilla con polea", "senatdilla con polea", "sentadilla con polea", "sentadillas con polea"] },
    { nombre: "Sentadilla en smith", videoUrl: "https://www.youtube.com/shorts/eFY7gyLFUcU", gifUrl: "https://static.exercisedb.dev/media/jFtipLl.gif", alias: ["Sentadilla en en smith", "Sentadilla en smith", "Sentadilla smith", "senatdilla en smith", "sentadilla en en smith", "sentadilla en smith", "sentadilla smith", "sentadillas en smith"] },
    { nombre: "Sentadilla goblet", videoUrl: "https://www.youtube.com/shorts/3wCp6MN2Z_Q", gifUrl: "https://static.exercisedb.dev/media/yn8yg1r.gif", alias: ["Sentadilla globet", "Sentadilla goblet", "senatdilla goblet", "sentadilla globet", "sentadilla goblet", "sentadillas goblet"] },
    { nombre: "Sentadilla maquina hack", videoUrl: "https://www.youtube.com/shorts/jXZyJ3KNbWw", gifUrl: "https://static.exercisedb.dev/media/Qa55kX1.gif", alias: ["Sentadilla maquina hack", "Sentadilla máquina hack", "senatdilla maquina hack", "sentadilla maquina hack", "sentadilla máquina hack", "sentadillas maquina hack"] },
    { nombre: "Sentadilla sumo", videoUrl: "https://www.youtube.com/shorts/rbKBfqqmmwY", gifUrl: "https://static.exercisedb.dev/media/dzz6BiV.gif", alias: ["Sentadilla sumo", "senatdilla sumo", "sentadilla sumo", "sentadillas sumo"] },
    { nombre: "Tiron al menton", videoUrl: "https://www.youtube.com/shorts/2SAyFkFCoFs", gifUrl: "https://static.exercisedb.dev/media/UDlhcO8.gif", alias: ["Tiron al menton", "tiron al menton"] },
    { nombre: "Triceps en polea con barra", videoUrl: "https://www.youtube.com/shorts/sU9snn0qTEs", gifUrl: "https://static.exercisedb.dev/media/3ZflifB.gif", alias: ["Triceps en polea con barra", "triceps en polea con barra", "tríceps en polea con barra"] },
    { nombre: "Triceps en polea con soga", videoUrl: "https://www.youtube.com/shorts/W7bQVLg3NWA", gifUrl: "https://static.exercisedb.dev/media/dU605di.gif", alias: ["Triceps en polea con soga", "triceps en polea con soga", "tríceps en polea con soga"] },
    { nombre: "Triceps polea 1 brazo", videoUrl: "https://www.youtube.com/shorts/hip11n3QXzU", gifUrl: "https://static.exercisedb.dev/media/qRZ5S1N.gif", alias: ["Triceps polea 1 brazo", "triceps polea 1 brazo", "tríceps polea 1 brazo"] },
    { nombre: "Twist", videoUrl: "https://www.youtube.com/shorts/vq1FICbQC4Q", gifUrl: "https://static.exercisedb.dev/media/XVDdcoj.gif", alias: ["Twist", "twist"] },
    { nombre: "Vitalizaciones", videoUrl: "https://www.youtube.com/shorts/gGIo7i-xnQA", alias: ["Vitalizaciones", "vitalizaciones"] },
    { nombre: "Vuelos frontales con barra", videoUrl: "https://www.youtube.com/shorts/QeJ7INwGgaE", gifUrl: "https://static.exercisedb.dev/media/b2Uoz54.gif", alias: ["Vuelos frontales con barra", "vuelos frontales con barra"] },
    ...
    { nombre: "Vuelos frontales con mancuernas", videoUrl: "https://www.youtube.com/shorts/ErPdiYXDeTw", gifUrl: "https://static.exercisedb.dev/media/3eGE2JC.gif", alias: ["Vuelos frontales con mancuerna", "Vuelos frontales con mancuernas", "vuelos frontales con mancuerna", "vuelos frontales con mancuernas"] },
    { nombre: "Vuelos frontales con polea", videoUrl: "https://www.youtube.com/shorts/AJ147wzeIfQ", gifUrl: "https://static.exercisedb.dev/media/u2X71Np.gif", alias: ["Vuelos frontales con polea", "vuelos frontales con polea", "Vuelos frontales en polea", "vuelos frontales en polea"] },
    { nombre: "Vuelos laterales con mancuernas", videoUrl: "https://www.youtube.com/shorts/dd9xo-_ahCg", gifUrl: "https://static.exercisedb.dev/media/DsgkuIt.gif", alias: ["Vuelos laterales con mancuerna", "Vuelos laterales con mancuernas", "vuelos laterales con mancuerna", "vuelos laterales con mancuernas", "Vuelos laterales", "vuelos laterales"] },
    { nombre: "Vuelos laterales en polea", videoUrl: "https://www.youtube.com/shorts/V98wql8IfVA", gifUrl: "https://static.exercisedb.dev/media/wEulIzp.gif", alias: ["Vuelos laterales en polea", "vuelos laterales en polea"] },

    { nombre: "Remo con mancuernas", videoUrl: "https://www.youtube.com/shorts/5QYQqPw_WgA", gifUrl: "https://static.exercisedb.dev/media/BJ0Hz5L.gif", alias: ["Remo con mancuernas", "remo con mancuernas", "remo mancuernas", "remo con mancuerna", "dumbbell row"] },
    { nombre: "Press arnold", videoUrl: "https://www.youtube.com/shorts/Kg8JD8l6ezw", gifUrl: "https://static.exercisedb.dev/media/Xy4jlWA.gif", alias: ["Press arnold", "press arnold", "arnold press", "press de arnold"] },
    { nombre: "Wall ball", videoUrl: "https://www.youtube.com/shorts/SDT1KrcO8ac", gifUrl: "https://static.exercisedb.dev/media/PsVS1QP.gif", alias: ["Wall ball", "wall ball", "wallball", "wall balls"] },
    { nombre: "Desplazamiento lateral con banda elastica", videoUrl: "https://www.youtube.com/shorts/N28Hpdezg7Q", gifUrl: "https://static.exercisedb.dev/media/O95afRA.gif", alias: ["Desplazamiento lateral con banda elastica", "Desplazamiento lateral con banda elástica", "desplazamiento lateral con banda elastica", "desplazamiento lateral con banda elástica", "desplazamiento lateral banda", "lateral band walk"] },
    { nombre: "Sit up", videoUrl: "https://www.youtube.com/shorts/V3MFEeDYYaE", gifUrl: "https://static.exercisedb.dev/media/2gPfomN.gif", alias: ["Sit up", "sit up", "sit-up", "situps", "sit ups"] },
    ...
    { nombre: "Press pallof", videoUrl: "https://www.youtube.com/shorts/iNn_sNA6TbU", gifUrl: "https://static.exercisedb.dev/media/9pa4H5m.gif", alias: ["Press pallof", "press pallof", "pallof press", "press de pallof"] },
    { nombre: "Rotacion de hombro con banda", videoUrl: "https://www.youtube.com/watch?v=fljC5LoRqxY", gifUrl: "https://static.exercisedb.dev/media/FWdVhcW.gif", alias: ["Rotacion de hombro con banda", "Rotación de hombro con banda", "rotacion de hombro con banda", "rotación de hombro con banda", "rotacion hombro banda", "rotaciones de hombro con banda"] },
    { nombre: "Rotacion externa de hombro 90", videoUrl: "https://www.youtube.com/shorts/iNn_sNA6TbU", gifUrl: "https://static.exercisedb.dev/media/x306lCW.gif", alias: ["Rotacion externa de hombro 90", "Rotación externa de hombro 90°", "Rotacion externa de hombro 90°", "rotacion externa de hombro 90", "rotacion externa hombro 90", "rotacion externa 90", "external rotation 90"] },
    { nombre: "Extension trasnuca", gifUrl: "https://static.exercisedb.dev/media/2IxROQ1.gif", alias: ["Extension trasnuca", "Extensión trasnuca", "extension trasnuca", "extensión trasnuca", "extension de triceps trasnuca", "extensión de tríceps trasnuca", "triceps trasnuca", "tríceps trasnuca", "extension tras nuca", "overhead tricep extension"] },
    { nombre: "Gemelos en prensa horizontal", gifUrl: "https://static.exercisedb.dev/media/Ie9UGty.gif", alias: ["Gemelos en prensa horizontal", "Gemelos en Prensa Horizontal", "gemelos en prensa horizontal", "pantorrillas en prensa horizontal", "gemelo en prensa horizontal", "calf press horizontal"] },
    { nombre: "Gemelos en maquina", gifUrl: "https://static.exercisedb.dev/media/bOOdeyc.gif", alias: ["Gemelos en maquina", "Gemelos en máquina", "gemelos en maquina", "gemelos en máquina", "gemelos maquina", "gemelos máquina", "pantorrillas en maquina", "pantorrillas en máquina", "gemelos sentado en maquina", "gemelos sentado en máquina", "calf machine", "seated calf raise"] },
    { nombre: "Gemelos en prensa 45°", alias: ["Gemelos en prensa 45°", "Gemelos en Prensa 45°", "Gemelos en prensa 45", "gemelos en prensa 45°", "gemelos en prensa 45", "pantorrillas en prensa 45°", "pantorrillas en prensa 45", "gemelo en prensa 45°", "calf raise 45"] },
    { nombre: "Flexiones", gifUrl: "https://static.exercisedb.dev/media/pvBMLHA.gif", alias: ["Flexiones", "flexiones", "flexiones de brazos", "flexion de brazos", "lagartijas", "push up", "push ups", "pushups"] },
    { nombre: "Abdominales", videoUrl: "https://www.youtube.com/shorts/V3MFEeDYYaE", gifUrl: "https://static.exercisedb.dev/media/2gPfomN.gif", alias: ["Abdominales", "abdominales", "ABS", "abs", "Abdominales / ABS", "abdominales / abs", "Abs", "abdominales tradicionales", "crunch", "crunches"] },
  ];
